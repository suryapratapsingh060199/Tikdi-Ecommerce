...
def send_mail(to, personalcode):
    ...
    sender_mail = f"{settings.EMAIL_HOST_USER}"
    password_sender = f"{settings.EMAIL_HOST_PASSWORD}"
    message = EmailMessage()
    message['To'] = to
    message['From'] = sender_mail
    message['Subject'] = "Welcome to Tikdi"
    message.set_content(
        f"Hello User welcome to Tikdi Your one time login link is\n {settings.SITE_URL}/verify/{personalcode} \nvalid for next 15 minutes.")
    try:
...
class DukanAuth:
            ...
            if user is None:
                ...
                if user.two_factor_auth:
                    ...
                    send_mail_celery.delay(
                        ...
                        subject=f'''Hey {user.first_name}! Your OTP is here for login on tikdi.''',
    ...
    def RegisterUser(self, request):
        ...
        send_mail_celery.delay(
            ...
            subject=f'''Hey {first_name}! Your Account Creation Link is here for tikdi.''',
    ...
    def RecoverAccount(self, request):
        ...
        if cache.get(email):
            ...
            send_mail_celery.delay(
                ...
                subject=f'''Dear {user.username}! Your Password Reset Link is here for tikdi.''',

                message=f"""Your One time Password Reset Link is http://127.0.0.1:8000/Verify/{token}/{email}.\n\nPlease Don't share the
                                    Link with anyone.\n\nThanks & Regards\nTeam Tikdi"""
            )
    ...
    def ValidateRecoverToken(self, request, *args, **kwargs):
        ...
        send_mail_celery.delay(
            ...
            subject=f'''Hey {user.username}! Your password has been changed on tikdi.''',

            message=f"""Hey {user.username}! Your password has been changed on tikdi.\n\n\n\nThanks&Regards\n\nApniDukan"""
        )
        cache.delete(email)
